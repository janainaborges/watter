<script lang="ts">
  ...
  async function handleNotificationsToggle(type: "water" | "break") {
    ...
    if (isActive) {
      await (type === "water" ? scheduleWaterNotifications : scheduleBreakNotifications)(notificationInterval);
    } else {
      const baseId = type === "water" ? 1000 : 2000;
        const numberOfNotifications = (Number(endTime.split(':')[0]) - Number(startTime.split(':')[0])) * 60 / notificationInterval; 
        const idsToCancel = Array.from({ length: numberOfNotifications }, (_, i) => baseId + i);
...
<div class="container">
  <div class="notifications">
    ...
    <div class="button">
      <ButtonComponent
        label={waterNotificationsActive ? "Desativar Notificações de Água" : "Ativar Notificações de Água"}
        on:click={toggleWaterNotifications}
        backgroundColor="#272d4d"
      />
      <ButtonComponent
        label={breakNotificationsActive ? "Desativar Notificações de Pausa" : "Ativar Notificações de Pausa"}
